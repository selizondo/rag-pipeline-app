...
**What breaks:** If ingest is running while a query arrives, the Chroma collection is in a partially-written state. The BM25 index (built at `HybridRetriever` startup) is also stale. It does not reflect in-flight ingest operations.
